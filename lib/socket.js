			...
			switch(MessageHex.substr(0,12)) { // Look for the first twelve bytes
				...
				case "686400a87274": // We've queried the socket for the name, and we've got data coming back
					if(hosts[index].name === "") { // If we haven't added the name of our socket to our array yet
						...
						strName = strName.substr(0,15) // And we want everything between our name and the next 202020202020 (our name is bookended between twenties!)
						// OLD: strName.split("2020202020")[0]; 
						// Name does always start after the 4th 20's but it's end is a maximum of 16 bytes padded by 20's. If the name is 16bytes then there are NO 20's.
						if(strName.toString('hex').substr(0,6) == "ffffff") {
							...
							strName = strName.trim(); //Trim trailing spcaes if any (20's).
						}
