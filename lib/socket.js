/*
  ...
  * Require the socket.js file, create a new instance of OrviboSocket, then call the prepare function, the discover function, the subscribe function, then setState
...
OrviboSocket.prototype.prepare = function(callback) { // Begin listening on our ports
...
OrviboSocket.prototype.discover = function(callback) { // To discover sockets, we send out the payload below. Any socket that is configured should report back.
...
OrviboSocket.prototype.subscribe = function(callback) { // We've found a socket, now we just need to subscribe to it so we can control it.
...
OrviboSocket.prototype.setState = function(index, state, callback) { // Here's where the magic begins! this function takes a boolean (state) and turns our socket on or off depending
	payload = [];
...
OrviboSocket.prototype.query = function(callback) { // Query all subscribed sockets for their name
...
OrviboSocket.prototype.getState = function(index, callback) { // Get the state of any given socket
